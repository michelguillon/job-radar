...
**Phase 6.5 — Persistence Hardening (SQLite, in progress).**
Interactive state (`activity_log`, `annotations`, `cv_tailor_links`) now
dual-writes to `corpus/job_radar.db` (SQLite, WAL mode) and reads from
it in the live overlay. Steps 1–5 complete; Step 6 (remove JSONL writes)
pending 1 week clean dual-write in production.
...
## 11.4 — Phase 6.5: Persistence Hardening ◐ in progress (Steps 1–5 ✅, Step 6 pending)

**Status:** Steps 1–5 complete and deployed. Step 6 (remove JSONL writes,
archive files) pending 1 week of clean dual-write in production.
...
- API writes dual-write to SQLite + JSONL
- API reads (live overlay) now come from SQLite
...
**Step 6 (pending — after 1 week dual-write):**
Remove JSONL writes from the three endpoints. JSONL files become
read-only audit archives.
