...
**Status:** Phases 1–6 complete — 527 tests. **Deployed + operational** at
...
**Phase 6.5 — Persistence Hardening (SQLite, ✅ complete).**
Interactive state (`activity_log`, `annotations`, `cv_tailor_links`) lives in
`corpus/job_radar.db` (SQLite, WAL mode) — the live overlay reads it and, as of
Step 6 (2026-06-19), every API write goes there only. The JSONL state files are
frozen read-only audit archives. All six steps complete; dual-write soaked cleanly
2026-06-14 → 06-19 (0 divergences) before the JSONL writes were removed.
See `docs/SPEC_DB_MIGRATION.md`.
...
## 11.4 — Phase 6.5: Persistence Hardening ✅ complete (all six steps)

**Status:** Complete (2026-06-19). SQLite is the sole interactive-state store;
the JSONL state files are frozen read-only audit archives.
Full spec: `docs/SPEC_DB_MIGRATION.md`.

**Scope (bounded — not a full rewrite):**
- Interactive state moves to SQLite: `activity_log`, `annotations`,
  `cv_tailor_links`
- Pipeline artefacts stay JSONL: raw, filtered, labelled, validated,
  scored corpus, calibration, stats.json
- Append-only discipline preserved — INSERT only, never UPDATE/DELETE

**What's built (all six steps):**
- `corpus/job_radar.db` (SQLite, WAL mode) with three tables
- Backfill from existing JSONL (idempotent)
- Dual-read comparison (`--source both`) confirmed 0 divergences on
  live corpus after fixing a `notes: None vs ''` edge case
- API reads (live overlay) come from SQLite
- `index.json` kept as pipeline cache (Option A) — interactive state
  from SQLite, pipeline artefacts from pre-built file
- CLI tools auto-detect SQLite via `use_sqlite()` (checks DB exists)
- **Step 6 (2026-06-19): JSONL writes removed — SQLite is the sole write
  destination.** All four API write paths (`workflow.py`, `annotations.py`,
  `cv_tailor.py`, `manual_ingest.py`'s owner-note) INSERT into SQLite only;
  the JSONL files are frozen read-only audit archives, never deleted.

**Step 6 cut-over:**
Dual-write soaked cleanly in production 2026-06-14 → 06-19 (5 days, 0
divergences), then the JSONL writes were removed. The JSONL *loaders* are
retained for `--source jsonl` and audit inspection.

**Notable as-built decisions:**
- SQLite chosen over PostgreSQL — single-user, home server, no new infra.
  PostgreSQL migration path documented in SPEC if multi-user is needed.
- `use_sqlite()` checks DB file existence — existence-based switch avoids
  a flag but requires deploy ordering: backfill BEFORE first serve.
- Annotation 409 now comes from SQLite UNIQUE constraint (not Python
  scan) — SQLite written first so the constraint fires before JSONL write.
- `notes: None` vs `notes: ''` divergence caught by dual-read gate on
  real prod data — regression test added.
